...
class GPT(nn.Module):
    ...
    def forward(
        ...
        if self.training:
            ...
            if speaker_embedding is not None:
                speaker_embedding = self.speaker_embedding_linear(speaker_embedding.permute(0, 2, 1).squeeze(1))
                his_latents += speaker_embedding.unsqueeze(1).expand(-1, his_latents.size(1), -1)

        else:
            speaker_embedding = self.speaker_embedding_linear(speaker_embedding[speaker_idx].permute(0, 2, 1).squeeze(1))
            his_latents += speaker_embedding.unsqueeze(1).expand(-1, his_latents.size(1), -1)
    ...
    def generate(
        ...
        **hf_generate_kwargs,
        ...
        # gpt_inputs = torch.tensor([[1,    1,    1,    1,    1,    1,    1,    1,    1,
