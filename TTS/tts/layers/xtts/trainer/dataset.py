...
def get_his_audio(sample):
        audiopath = sample['audio_file']
        if sample['language'] == 'en':
            audioid = int(audiopath.split('-')[-1].split('.')[0])
        if sample['language'] == 'zh-cn':
            audioid = int(audiopath.split('_')[-1].split('.')[0].replace('S', '0'))

        if audioid < 2:
            return [audiopath, audiopath]
        
        elif audioid == 2:
            if os.path.exists(audiopath.replace(str(audioid).zfill(4), str(audioid-1).zfill(4))):
                return [audiopath.replace(str(audioid).zfill(4), str(audioid-1).zfill(4)),
                audiopath.replace(str(audioid).zfill(4), str(audioid-1).zfill(4))]
            else:
                return [audiopath, audiopath]

        else:
            if os.path.exists(audiopath.replace(str(audioid).zfill(4), str(audioid-1).zfill(4))) and os.path.exists(audiopath.replace(str(audioid).zfill(4), str(audioid-2).zfill(4))):
                return [audiopath.replace(str(audioid).zfill(4), str(audioid-1).zfill(4)), 
                audiopath.replace(str(audioid).zfill(4), str(audioid-2).zfill(4))]
...
class XTTSDataset(torch.utils.data.Dataset):
    def __init__(self, config, samples, tokenizer, sample_rate, is_eval=False):
        ...
        #     input_sample_rate=model_args.input_sample_rate,
        #     output_sample_rate=model_args.output_sample_rate,
        #     output_hop_length=model_args.output_hop_length,
        #     ar_mel_length_compression=model_args.gpt_code_stride_len,
        #     decoder_input_dim=model_args.decoder_input_dim,
        #     d_vector_dim=model_args.d_vector_dim,
        #     cond_d_vector_in_each_upsampling_layer=model_args.cond_d_vector_in_each_upsampling_layer,
        # )
    ...
    def __getitem__(self, index):
        ...
        try:
            ...
            #history style load
            his_audio = get_his_audio(sample)
            his_cond = []
            for i in his_audio:
                cond, _, cond_idxs = get_prompt_slice(
                    i, self.max_conditioning_length, self.min_conditioning_length, self.sample_rate, self.is_eval
                )
                his_cond.append(cond)
            his_cond = torch.stack(his_cond)

        #     audio_16k = torchaudio.functional.resample(wav, 24000, 16000)
        #     speaker_embedding = (
        #         self.hifigan_decoder.speaker_encoder.forward(audio_16k.to('cuda'), l2_norm=True)
        #         .unsqueeze(-1)
        #         .to(self.device)
        # )
            # samples[lang][index]['cond'] = cond
    ...
    def collate_fn(self, batch):
        ...
        batch["his_cond"] = torch.stack(batch['his_cond'])
        batch['sample_idx'] = torch.stack(batch['sample_idx'])
