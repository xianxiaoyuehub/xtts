...
class GPTTrainer(BaseTTS):
    ...
    @torch.no_grad()
    def compute_his_mel(self, batch):
        B, num_cond_samples, C, T = batch["his_cond"].size()
        conditioning_reshaped = batch["his_cond"].view(B * num_cond_samples, C, T)
        his_conditioning_mel = self.torch_mel_spectrogram_style_encoder(conditioning_reshaped)
        ...
        T_mel = his_conditioning_mel.size(2)
        his_conditioning_mel = his_conditioning_mel.view(B, num_cond_samples, n_mel, T_mel)
    ...
    def get_speaker_embedding(self, batch):
        ...
        for i in range(batch_size):
            emb = self.xtts.get_speaker_embedding(batch['wav'][i], 24000).squeeze(0)
            speaker_embedding.append(emb)
    ...
    def format_batch_on_device(self, batch):
        ...
        batch['his_cond_mels'] = self.compute_his_mel(batch)
        # self.xtts.gpt.sample['en'][batch['sample_idx']]['cond_mels'] = paired_conditioning_mel
    ...
    def get_data_loader(
        ...
        if is_eval and not config.run_eval:
            ...
            # XTTS_CHECKPOINT_LINK = '/workspace/TTS/recipes/ljspeech/xtts_v2/run/training/XTTS_v2.0_original_model_files/model.pth'
            # checkpoint = load_fsspec(XTTS_CHECKPOINT_LINK)

            # pretrained_dict = {k[16:] : v for k, v in checkpoint['model'].items() if 'hifigan_decoder' in k}
            # dataset_dict = dataset.hifigan_decoder.state_dict()
            # dataset_dict.update(pretrained_dict)
            # dataset.hifigan_decoder.load_state_dict(dataset_dict)
            ...
            # self.xtts.gpt.sample = dataset.samples
            # dataset.model = self.xtts
